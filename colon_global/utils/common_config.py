...
def get_model(p, pretrain_path=None, mlp_number=None, cls_head_number=None ):
    # Get backbone
    if p['backbone'] == 'resnet18':
    ...
    if p['setup'] in ['simclr', 'moco']:
        ...
        model = ContrastiveModel(backbone, mlp_number=mlp_number, cls_head_number=cls_head_number, **p['model_kwargs'])
...
def get_train_dataset(p, transform, gcloud=False, to_augmented_dataset=False, class_num=0,
                        to_neighbors_dataset=False, split=None):
    # Base dataset
    if p['train_db_name'] == 'cifar-10':
        from data.lag_loader import lag_Dataset
        from gcloud import download_colon_unzip, download_colon_perm_unzip, download_lag_perm_unzip, download_lag_rotate_unzip

        if gcloud:
            root_dir = '/pvc/lag_rotate'
            download_lag_rotate_unzip(root_dir)
        else:
            root_dir = '/home/yu/lag_rotate'


        dataset = lag_Dataset(root_dir=root_dir, train=True, transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.cifar import CIFAR20s
