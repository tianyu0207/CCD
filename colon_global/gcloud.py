...
def download_colon_unzip(data_dir: str):
    bucket_prefix = 'Colon_AD_public.zip'
    ...
    if dst_folder.exists():
        ...
        with open(Path('/pvc')/bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        with zipfile.ZipFile('/pvc/Colon_AD_public.zip', 'r') as zip_ref:
            zip_ref.extractall('/pvc/')
    print(os.listdir('/pvc/'))

def download_colon_rotate_unzip(data_dir: str):
    bucket_prefix = 'rotate_augment.zip'
    dst_folder = Path(data_dir)
    if dst_folder.exists():
        print(os.listdir('/pvc/'))
        print('Skipping download as data dir already exists')
        ...
        with open(Path('/pvc') / bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        with zipfile.ZipFile('/pvc/rotate_augment.zip', 'r') as zip_ref:
            zip_ref.extractall('/pvc/')
    print(os.listdir('/pvc/'))


def download_lag_rotate_unzip(data_dir: str):
    bucket_prefix = 'lag_rotate.zip'
    dst_folder = Path(data_dir)
    if dst_folder.exists():
        print(os.listdir('/pvc/'))
        print('Skipping download as data dir already exists')
        ...
        with open(Path('/pvc') / bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        with zipfile.ZipFile('/pvc/lag_rotate.zip', 'r') as zip_ref:
            zip_ref.extractall('/pvc/')
    print(os.listdir('/pvc/'))

def download_lag_perm_unzip(data_dir: str):
    bucket_prefix = 'lag_perm.zip'
    dst_folder = Path(data_dir)
    if dst_folder.exists():
        print(os.listdir('/pvc/'))
        print('Skipping download as data dir already exists')
        ...
        with open(Path('/pvc') / bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        with zipfile.ZipFile('/pvc/lag_perm.zip', 'r') as zip_ref:
            zip_ref.extractall('/pvc/')
    print(os.listdir('/pvc/'))

def download_colon_perm_unzip(data_dir: str):
    bucket_prefix = 'perm_augment.zip'
    dst_folder = Path(data_dir)
    if dst_folder.exists():
        print(os.listdir('/pvc/'))
        print('Skipping download as data dir already exists')
        ...
        with open(Path('/pvc') / bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        with zipfile.ZipFile('/pvc/perm_augment.zip', 'r') as zip_ref:
            zip_ref.extractall('/pvc/')
    print(os.listdir('/pvc/'))
    ...
    # # print(os.listdir('/pvc/'))
    # print('downloading')
    # bucket = get_bucket('aiml-carneiro-research', 'aiml-carneiro-research-data')
    # blob = bucket.blob(bucket_prefix)
    # with open(Path('/pvc')/bucket_prefix, 'wb') as sink:
    #     blob.download_to_file(sink)
    # print('unziping')
