...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'lag']:
...
def get_train_dataset(p, transform, pos_transform, to_augmented_dataset=False,class_num=0,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        print('class number is: '+ str(class_num) + '!!!!!!!!!!!!!!!!!!')
        print('class number is: '+ str(class_num) + '!!!!!!!!!!!!!!!!!!')
        print('class number is: ' + str(class_num) + '!!!!!!!!!!!!!!!!!!')
        idx = dataset.targets == class_num
        dataset.targets = dataset.targets[idx]
        dataset.data = dataset.data[idx.numpy().astype(np.bool)]
        aug_data = rotate_array_deterministic(dataset.data)
        lab_data = np.concatenate([dataset.targets for _ in range(4)], axis=0)

        dataset.data = aug_data
        dataset.targets = lab_data
        dataset.targets = torch.tensor(dataset.targets)

    elif p['train_db_name'] == 'lag':
        from inbreast import inbreast_Dataset, breast_pos_Dataset
        root_dir = '/home/yu/inbreast/'
        print(root_dir)
        dataset = inbreast_Dataset(root_dir= root_dir, train=True, transform=transform)
        # dataset.targets = torch.tensor(dataset.targets)
        ...
        # dataset.data = aug_data
        # pos_Dataset = breast_pos_Dataset(root_dir=root_dir, train=True, transform=pos_transform)
    ...
    if to_augmented_dataset: # Dataset returns an image and an augmentation of that image.
        ...
        # pos_Dataset = AugmentedDataset(pos_Dataset)
...
def get_train_dataloader(p, dataset, pos_dataset):
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'],
                                     batch_size=p['batch_size'], pin_memory=False, collate_fn=collate_custom,
                                     drop_last=True, shuffle=True), torch.utils.data.DataLoader(pos_dataset,
                                                                                                num_workers=p[
                                                                                                  'num_workers'],
                                                                                                batch_size=p[
                                                                                                  'batch_size'],
                                                                                                ...
                                                                                                drop_last=True,
                                                                                                shuffle=True)  # should pos need shuffle?
